...
enum class ReliableKind : uint8_t {
    ...
    WaterState = 128,  // 2026-08-30 (v148, authored on v143): bucket fill state (prop_bucket_C::height) and sponge
                       ...
                       //     ue_wrap/water_prop.
                       //     2026-08-31 CATCH-UP: renumbered 125 -> 128 and kProtocolVersion bumped
                       //     to 148. The lane was authored against v143, where 125 was free; the
                       //     upstream admission exchange (AuthHello/Challenge/Proof) has since
                       //     taken 125-127, so the original number would have collided with the
                       //     handshake. The earlier "stays 143" deviation note is retired: the
                       //     bump is taken here because this fork rebased onto upstream v147.
...
static_assert(sizeof(AuthChallengePayload) == 96, "AuthChallengePayload must be 96 bytes");

struct AuthProofPayload {
    uint8_t sig[kAuthSigBytes];      // 64 -- client's signature over the host's nonce blob
};
static_assert(sizeof(AuthProofPayload) == 64, "AuthProofPayload must be 64 bytes");
