...
enum class ReliableKind : uint8_t {
    ...
    DishAimState = 55, // 2026-06-12 (v64; v109: -viewCoordinate to DeskCursorPose; v144: +flags):
                       //     the coords-panel committed aim locks (Coordinate_0..2 + selected +
                       //     the v70 Direction toggle + v144 flags: bits 0..2 activeCoord_0..2,
                       //     bit 3 coordsInPlace, bit 7 snapshot) change-gated / connect-snapshotted
                       //     by the desk holder; host-relayed. Receivers raw-write + reflected
                       //     updCursorLocations (which also rotates the physical pingDishes).
                       //     Payload: DishAimStatePayload (32 B). Module: coop/console_state_sync.
...
// flags bits for DishAimStatePayload.flags
namespace dishaim_flags {
inline constexpr uint8_t kActiveCoord0  = 0x01;  // bit 0: activeCoord_0
inline constexpr uint8_t kActiveCoord1  = 0x02;  // bit 1: activeCoord_1
inline constexpr uint8_t kActiveCoord2  = 0x04;  // bit 2: activeCoord_2
inline constexpr uint8_t kCoordsInPlace = 0x08;  // bit 3: coordsInPlace (triangle render gate)
inline constexpr uint8_t kSnapshot      = 0x80;  // bit 7: host connect snapshot (valid from host when holder==0xFF)
}  // namespace dishaim_flags
...
struct DishAimStatePayload {
    ...
    uint8_t flags;                   // 1  -- v144: dishaim_flags (bit 0..2 = activeCoord_0..2, bit 3 = coordsInPlace, bit 7 = snapshot)
    uint8_t _pad[2];                 // 2
};
static_assert(sizeof(DishAimStatePayload) == 32, "DishAimStatePayload must be 32 bytes (v109: -viewCoordinate, v144: +flags)");
