...
| Refiner pane (comp) | `comp_progress` refine (NOT the download) | 4 single-sim | occupant | `CompState 60` / `CompData 61` | **AS-BUILT** (simultaneous writers diverge permanently: the host relay skips the origin at `src/votv-coop/src/coop/net/session_relay.cpp:80`, so C1=A + C2=B interleaved A->B leaves C2 at A with its local baseline matched and never repaired. See the R17 attempt notes under Comp-processing in `COOP_SYNC_PROFILES.md` for five further silent-loss paths found in this lane on 2026-08-29) |
