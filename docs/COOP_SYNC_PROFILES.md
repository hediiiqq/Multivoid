...
| 9 | dish committed-aim locks | B | HO | CA | `console_state_sync::OnDishAim` **[take-4 R3. Symptom is SYMMETRIC, not host-only: hands-on 2026-08-29, the peer who commits sees its own triangle and ANY observer sees none, both directions. Measured with desk_diag on two peers: c0/c1/c2, selected and Direction arrive BYTE-IDENTICAL, so send/relay/holder-gate/apply are all clean and the state is not the problem. GATE HYPOTHESIS TRIED AND REFUTED: a build replicating activeCoord_0..2 + coordsInPlace (written before updCursorLocations) still drew nothing, so opening the gate is not sufficient and that build was reverted. Surviving suspects: updCursorLocations does NOT re-derive triangle_position/triangle_size/perim/Line_0..2/coordinate_middle on a peer that never ran movePointToCursor, or the per-edge dynmat MIDs are never repainted there. movePointToCursor(selected:int, onCooldown:bool) is PE-callable and is the untried route -- it costs the observer coord_cooldown and a click sound]** | connect snapshot |
